...
<form method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>"   class="header-search-form d_flex flex-nowrap icon-fix <?php echo esc_attr($class); ?> <?php if($category) echo 'cat-dropdown'; ?>">    
       <input id="<?php echo esc_attr($unique_id); ?>" type="text" placeholder="<?php esc_attr_e( 'Type here...', 'wooxon' ); ?>" autocomplete="off" class="w100 order-2 border-no" value="<?php echo get_search_query(); ?>" name="s" required />
